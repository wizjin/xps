...
XPS_INLINE void xps_input_tcp_close(xps_input_t *input) {
    xps_input_tcp_t *l = xps_structof(input, xps_input_tcp_t, input);
    if (l->fd != INVALID_SOCKET) {
        l->core->evacts->del(l->core->evacts, &l->event);
        xps_tcp_close(l->fd);
        l->fd = INVALID_SOCKET;
    }
}

XPS_INLINE void xps_input_tcp_event_handler(xps_event_t *ev) {
    xps_input_tcp_t *l = xps_structof(ev, xps_input_tcp_t, event);
    if (ev->eof) {
        log_error("listen failed, force close: %p", l);
        xps_input_tcp_close(&l->input);
    } else {
        int n = ev->available;
        for (int i = 0; i < n; i++) {
            ...
            int fd = xps_tcp_accept(l->fd, (struct sockaddr *)&addr, &socklen);
            if (fd == INVALID_SOCKET) {
                log_warn("accept new connect failed: %s", strerror(errno));
                break;
            }
            xps_connection_t *conn = l->accept(l->core, fd);
            if (conn == NULL) {
